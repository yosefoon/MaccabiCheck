...
ושולח התראה בטלגרם — ואופציונלית גם בוואטסאפ וגם **שיחה קולית מצלצלת בטלגרם** — כשהתאריך של "תור פנוי קרוב" עומד בתנאי שהוגדר.
...
קבצים:
- `check.mjs` — הבדיקה עצמה: מושך את דף הרופא, מחלץ את התאריך שליד "תור פנוי קרוב", משווה ליעד ושולח התראות (טלגרם; וואטסאפ ושיחה קולית אם הוגדרו).
...
- `.env` (מקומי בלבד, לא בגיט!) — סודות הערוצים: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`; אופציונלית וואטסאפ דרך CallMeBot: ‏`WHATSAPP_PHONE` (בפורמט ‎+972...) ו-`CALLMEBOT_APIKEY`; ואופציונלית שיחה קולית בטלגרם: `TELEGRAM_CALL_USER` (שם המשתמש, למשל ‎@username; דורש ‎`/start` חד-פעמי ל-‎@CallMeBot_txtbot). כל ערוץ שמוגדר — משתתף בהתראות.
...
| `whatsappRepeatMinutes` | מרווח נפרד לערוץ הוואטסאפ. `0` = כל בדיקה (ההגדרה הנוכחית). ‏CallMeBot החינמי מגביל קצב (503) — הודעות שנחסמו מנוסות שוב בבדיקה הבאה; אם רוצים פחות רעש, להעלות ל-5 ומעלה. טלגרם לא מושפע |
| `callRepeatMinutes` | מרווח בין שיחות מצלצלות (ברירת מחדל 3 דק'). שיחה אורכת עד ~30 שנ', לכן אין טעם ב-0 — שיחות היו מתנגשות זו בזו. חל רק על התראת 🚨 (לא על ⚠️ תקלה) |
...
TEST_CALL=1 node check.mjs     # שיחת בדיקה מצלצלת בטלגרם (אם TELEGRAM_CALL_USER מוגדר)
...
הקמה מחדש של הלולאה המקומית (מחשב חדש / אחרי מחיקה): ליצור `.env` עם הסודות (שניים של טלגרם; אופציונלית שניים של וואטסאפ ו-`TELEGRAM_CALL_USER` לשיחות), ואז להריץ פעם אחת את `register-task.ps1` ב-PowerShell (במחשב אישי) או את `register-task-server.ps1` (בשרת תמיד-דולק).
...
**שלב 2 — הורדת הקוד** (אותו בלוק משמש גם לעדכון גרסה בעתיד; `.env`, ‏state ולוגים שורדים — אבל `config.json` נדרס בגרסת הריפו, אז עריכות מקומיות בשרת יש לעשות מחדש או לשמור קודם בריפו. עדכון שנוגע ב-`watch.mjs` דורש בנוסף `Stop-ScheduledTask` + `Start-ScheduledTask` — הלולאה הרצה לא קוראת אותו מחדש לבד):
...
**שלב 3 — סודות** (הערכים אצלך, לא בריפו! לא דרך Notepad — הוא מוסיף `.txt` בשקט). שלוש השורות האחרונות אופציונליות — וואטסאפ דורש מפתח CallMeBot, ושיחות דורשות `/start` חד-פעמי ל-‎@CallMeBot_txtbot בטלגרם; למחוק שורות שלא בשימוש:
...
TELEGRAM_CALL_USER=<@שם המשתמש בטלגרם>
"@ | Set-Content -Path 'C:\MaccabiCheck\.env' -Encoding Ascii
```

(שדרוג שרת קיים להוספת ערוץ = להריץ שוב את בלוק שלב 2 בשביל הקוד החדש + להוסיף את השורות הרלוונטיות ל-`.env`; אם העדכון נגע ב-`watch.mjs` — גם `Stop-ScheduledTask 'MaccabiCheck Watcher'` ואז `Start-ScheduledTask 'MaccabiCheck Watcher'`.)
...
$env:TEST_CALL='1'; node .\check.mjs; Remove-Item Env:\TEST_CALL      # שיחת בדיקה מצלצלת (אם TELEGRAM_CALL_USER מוגדר)
```
